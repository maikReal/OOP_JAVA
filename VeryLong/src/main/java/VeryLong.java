...
public class VeryLong {
    ...
    private List<Integer> toArrayList(int[] arr) {
        ...
        List<Integer> primArr = new ArrayList<Integer>();
    ...
    public VeryLong division(VeryLong val) {
        VeryLong a = this.abs();
        VeryLong b = val.abs();
        VeryLong res = new VeryLong(0);

        while (a.bigger(b,true)) {
            a = a.substraction(b);
            res.increment();
        }

        if (this.symbol.equals("-") || val.symbol.equals("-")) {
            res.symbol = "+";
            return res;
        }
        return res;
    ...
    private VeryLong calcMultiplication(int[] bigNum, int[] smallNum, List<Integer> bigRemainder, int numberZeros) {
        ...
        List<Integer> resRem = new ArrayList<Integer>();
    ...
    public VeryLong multiply(VeryLong num) {
        ...
        List<Integer> bigRemainder = new ArrayList<Integer>();
        String resSymbol;

        if ((this.symbol.equals("-") && num.symbol.equals("-")) ||
                (this.symbol.equals("+") && num.symbol.equals("+"))) {
            resSymbol = "+";
        } else {
            resSymbol = "-";
        }
        ...
        if (this.number.length > num.number.length) {
            ...
            VeryLong calculationResult = calcMultiplication(this.number, num.number, bigRemainder, 0);
            calculationResult.symbol = resSymbol;
            ...
            VeryLong calculationResult = calcMultiplication(num.number, this.number, bigRemainder, 0);
            calculationResult.symbol = resSymbol;
    ...
    public VeryLong substraction(VeryLong num) {
        ...
        while (mainIndex >= 0) {
            ...
            if (remainderSub != 0) {
                ...
                if (resSub < 0) {
                    resSub = num1[mainIndex] + 10 - remainderSub;
    ...
    public VeryLong add(VeryLong num) {
        ...
        if (this.symbol.equals("-")) {
            ...
            boolean isFirstBigger = this.bigger(num, true);
    ...
    public String toString() {
        ...
        boolean hasZerosAtBeginning = true;
        for (int i : this.number) {

            if (i != 0) {
                hasZerosAtBeginning = false;
            }

            if (!hasZerosAtBeginning) {
                val.append(i);
            }
        }
